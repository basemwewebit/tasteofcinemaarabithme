...
rotation_test_check(mazaq_listicle_heading_numbers([]) === []);

// Prose article, no numbers anywhere → positional 1..N.
rotation_test_check(mazaq_listicle_heading_numbers(['المقدمة', 'الحبكة', 'الخاتمة']) === [1, 2, 3]);
...
rotation_test_check(mazaq_listicle_heading_numbers(['1. Alien', '2. Aliens']) === [1, 2]);
...
rotation_test_check(mazaq_listicle_heading_numbers(['9) Alien', '8 - Aliens', '7 – Alien 3', '6 — Resurrection']) === [9, 8, 7, 6]);
...
rotation_test_check(mazaq_listicle_heading_numbers(['1917 (2019)', '2001: A Space Odyssey', 'مقدمة']) === [1, 2, 3]);
...
rotation_test_check(mazaq_listicle_heading_numbers(['مقدمة', '3. Ratatouille', '2. Monsters, Inc.', '1. Toy Story'])
    === [null, 3, 2, 1]);
...
rotation_test_check(mazaq_listicle_heading_numbers(['مقدمة', 'خاتمة', '2. Aliens', '1. Alien']) === [null, null, 2, 1]);
...
rotation_test_check(mazaq_listicle_heading_numbers(['100. Alien', '101. Aliens', '1000. Movies']) === [100, 101, null]);
...
rotation_test_check(mazaq_listicle_heading_numbers(['١١. Up (2009)', '١٠. Inside Out']) === [1, 2]);

// Mixed scripts: only the ASCII rank counts, so the Arabic-numbered one shows nothing
// rather than a bogus 00.
rotation_test_check(mazaq_listicle_heading_numbers(['١١. Up (2009)', '10. Inside Out']) === [null, 10]);
