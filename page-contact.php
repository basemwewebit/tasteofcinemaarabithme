...
<div class="bg-slate-900 text-white py-16 md:py-24 mb-12 relative overflow-hidden text-center">
    <div class="max-w-4xl mx-auto px-4 relative z-10 w-full">
        <h1 class="text-display mb-6 text-white">اتصل بنا</h1>
        <p class="text-xl text-slate-300 font-medium max-w-2xl mx-auto">نحن دائماً هنا للاستماع إليك!</p>
    </div>
</div>

<main id="main-content" class="flex-1 max-w-7xl mx-auto px-4 py-8 mb-16 w-full">
        ...
        <div class="w-full lg:w-2/3 bg-slate-100 dark:bg-slate-800 rounded-2xl p-8 border border-slate-200 dark:border-slate-700">
            ...
            <form method="post" class="flex flex-col gap-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="contact-name" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">الاسم الكامل</label>
                        <input type="text" id="contact-name" name="name" placeholder="الاسم الكامل" required class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 outline-none focus:border-primary text-slate-800 dark:text-white">
                    </div>
                    <div>
                ...
                <div>
                    <label for="contact-subject" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">الموضوع</label>
                    <input type="text" id="contact-subject" name="subject" placeholder="الموضوع" required class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 outline-none focus:border-primary text-slate-800 dark:text-white">
                </div>
                <div>
                    <label for="contact-message" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">نص الرسالة</label>
                    <textarea id="contact-message" name="message" rows="6" placeholder="نص الرسالة" required class="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 outline-none focus:border-primary text-slate-800 dark:text-white resize-none"></textarea>
                </div>
                <button type="submit" name="mazaq_contact_submit" value="1" class="bg-primary hover:bg-slate-900 dark:hover:bg-white text-slate-900 hover:text-white dark:hover:text-slate-900 font-bold text-lg px-8 py-4 rounded-xl transition-colors shadow-md self-start w-full md:w-auto">إرسال الرسالة</button>
            </form>
        </div>
